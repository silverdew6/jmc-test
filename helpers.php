...
if ( ! function_exists('appc_tax'))
{
    /**
     * 查询业务模式.
     */
    function appc_tax ($value)
    {
             $filter['id']=$value;
		     $objTax = app::get('sysshop')->model('tax'); //2016/4/7  lcd
		     $taxname = $objTax ->getRow('name',$filter);
           return $taxname['name'];
    }
}


if ( ! function_exists('appc_region'))
{
    /**
     * 查询区域
     */
    function appc_region ($value)
    {
             $filter['id']=$value;
             $objRegion= app::get('sysshop')->model('ware_region');//2016/4/7 lcd  修改为ware_region
		     //$objRegion= app::get('ectools')->model('region');//2016/4/7 lcd  修改为ware_region
			 $regionname = $objRegion->getRow('name',$filter);
